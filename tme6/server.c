...
int main(int argc, char *argv[])
{
        ...
        socklen_t clilen;                                           // Creation de structure client
        char buffer[256];
        ...
        while (1) {
                newsockfd = accept(sockfd, (struct sockaddr *) &cli_addr, &clilen);         // Acception de la connection
                if (newsockfd < 0)
                    error("ERROR on accept");

                bzero(buffer, 256);                                 // Initialise the buffer in 0
