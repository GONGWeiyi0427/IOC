...
{
        perror(msg);                                                // Affichier l'error correspondant
...
int main(int argc, char *argv[])
{
        ...
        struct hostent *server;                                     // Structure avec nom de l'hote, tableau de noms alternatifs pour l'hote, type d'adresse de l'hote, longueur de l'adresse de l'hote, tableau d'adreses IP pour l'hote
        ...
        if (sockfd < 0)                                             // Si le socket est null
        ...
        if (connect(sockfd,(struct sockaddr *) &serv_addr,sizeof(serv_addr)) < 0)                   // Connect to the serveur
        ...
        n = write(sockfd,buffer,strlen(buffer));                    // Ecrire le contenu de buffer dans le socket
        if (n != strlen(buffer))                                    // Si la loneur n'est pas la meme
